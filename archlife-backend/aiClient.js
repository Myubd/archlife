 ...
 * @param {string} [opts.anonId] - useExternal=trueの場合は必須(サーバー側で1日の利用回数上限を
 *   匿名ID単位でチェックするため。cryptoStorage.jsのcreateStorage()が返すanonIdを渡す)。
 */
export async function analyzeWithBackend({ apiBaseUrl, kind, payload, useExternal, provider, anonId }) {
