...
`ArchLifeOS_VercelRenderNeon_Architecture.md` を参照してください。
...
- RenderにはGPUプランがないため、ローカルLLMはCPUでの運用になる(既定は`qwen3:8b`だが、
  starterプランだと応答がかなり遅い/メモリ不足になりうるため、必要に応じて`qwen2.5:1.5b-instruct`等の
  小型モデルに変更することを推奨)。AWS版(EC2+GPU)ほどの応答品質・速度は期待しにくい
